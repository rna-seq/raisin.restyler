...
test: bin/nosetests bin/gvizapi
	bin/nosetests -s raisin/restyler

coverage: bin/coverage bin/nosetests
	bin/nosetests --with-coverage --cover-html --cover-html-dir=html --cover-package=raisin.restyler
	bin/coverage html

pylint:	bin/pylint
	bin/pylint -i y raisin/restyler

flake8:	bin/flake8
	bin/flake8 --max-complexity 12 raisin/restyler

pep8:	bin/pep8
	bin/pep8 raisin/restyler

pyflakes:	bin/pyflakes
	bin/pyflakes raisin/restyler
...
templer: bin/python
	...
	export OLDHOME="${HOME}"; export HOME="${PWD}"; ./bin/templer dotpackage raisin.restyler; export HOME="${OLDHOME}"

diff: bin/python
	# Show the difference between the current package and the regenerated one
	colordiff -c -r raisin.restyler .|less -r

sloccount:	bin/python
	sloccount raisin/restyler
